...
public class InventoryServiceImpl implements InventoryService {
    ...
    private final int DEFAULT_INVENTORY_SIZE = 27;
    @Getter
    ...
    public boolean addItem(String itemId, int count) {
        String username = worldService.getPlayerData("").getUsername(); // Get current player
        ...
        if (success) {
            ...
            // Handle multiplayer sync if needed
    ...
    public void moveItem(int fromSlot, int toSlot) {
        ...
        if (worldService.isMultiplayerMode()) {
            ...
            action.setCount(toSlot); // Using count to store target slot
            multiplayerClient.sendMessage(action);
        }
    }
